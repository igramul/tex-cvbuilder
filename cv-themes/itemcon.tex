...
% adjust Custom commands for CV
\renewcommand{\sitemtit}{\color{bodytext}\sffamily}

% https://tex.stackexchange.com/questions/368470/dot-list-with-a-line-linking-the-dots
\newlist{itemcon}{itemize}{1}

\newcounter{itemdot}
\DeclareRobustCommand{\dottedlinked}[1]{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[draw,anchor=base,inner sep=0pt,outer sep=0pt,circle,{#1},line width=.75pt,yshift=0.3ex, xshift=-.5ex] (itemdot\number\value{itemdot}) {\phantom{\textbullet}};
    \ifnum0<\value{itemdot}
    \draw[bulletcolor,line width=.75pt,] (itemdot\number\value{itemdot}) -- (itemdot\the\numexpr\value{itemdot}-1);
    \fi
    \stepcounter{itemdot}%
  \end{tikzpicture}%
}
\makeatletter
\g@addto@macro{\itemcon}{\setcounter{itemdot}{0}} % reset the itemdot counter for a new itemize environment
\makeatother

% adjust Bullet style and spacing
\setlist[itemcon,1]{label={\dottedlinked{bulletcolor}},leftmargin=2.5mm}
\setlist[itemize,1]{label={},parsep=2pt,labelindent=0pt, leftmargin=*}
\setlist[itemize,2]{label={\color{bulletcolor}$\bullet$},parsep=2pt,labelindent=0pt}
\setlist[itemize,3]{label={\color{bulletcolor}$\bullet$},noitemsep,labelindent=0pt}
...
\newcommand{\makeprofile}{%
    ...
    \ifthenelse{\equal{\profilepic}{}}{}{%
        \begin{tikzpicture}[remember picture,overlay]
            \node[rectangle, minimum height=4.5cm, minimum width=4.5cm, path picture={
                \node at (path picture bounding box.center){\includegraphics[width=4.5cm]{\profilepic}};}] at ($(current page.north west)+(3.5cm,-5.5cm)$) {};
        \end{tikzpicture}
    }
}
